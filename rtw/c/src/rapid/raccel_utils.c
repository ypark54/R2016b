 ...
 */
int rt_GetNumStructLeaves(uint16_T dTypeMapIdx, 
                          uint16_T dimMapIdx,
                          uint16_T fixPtIdx)
{
    ...
    /* found a non-fixed-point struct field; recurse */
    {
        ...
        for (loopIdx=0; loopIdx < dTypeMapNumElems; loopIdx++) {
            elDTypeMapIdx = rtwCAPI_GetElementDataTypeIdx(elemMap, dTypeMapElemMapIdx+loopIdx);
            elDimMapIdx   = rtwCAPI_GetElementDimensionIdx(elemMap, dTypeMapElemMapIdx+loopIdx);
            elFixPtIdx    = rtwCAPI_GetElementFixPtIdx(elemMap, dTypeMapElemMapIdx+loopIdx);
                                                  ...
                                                  const char **result)
{
    ...
    *numNonStructDataTypes = 0;
    ...
    const rtwCAPI_ModelParameters *mdlPrms    = rtwCAPI_GetModelParameters(mmi);
        ...
        for (loopIdx=0; loopIdx<nTrans; loopIdx++) {
            mxArray  *valueMat                = NULL;
            mxArray  *structParamInfo         = NULL;

            valueMat = mxGetField(paParamStructs,loopIdx,"values");
         ...
         */
            ...
            structParamInfo = mxGetField(paParamStructs, loopIdx, "structParamInfo");
...
{
    int numFields                = mxGetNumberOfFields(structArray);
    const char **fieldNames      = (const char **) malloc(sizeof(const char *) * numFields);
                          ...
                          const char** result)
{
    ...
    if (dTypeMapNumElems == 0 || fixPtIdx > 0) {
...

        {
            int loopIdx;
            for (loopIdx=0; loopIdx<dTypeMapNumElems; loopIdx++) {       
                uint16_T         elDTypeMapIdx         = rtwCAPI_GetElementDataTypeIdx(elemMap, dTypeMapElemMapIdx+loopIdx);
                uint32_T         elOffset              = rtwCAPI_GetElementOffset(elemMap, dTypeMapElemMapIdx+loopIdx);
                uint16_T         elFixPtIdx            = rtwCAPI_GetElementFixPtIdx(elemMap, dTypeMapElemMapIdx+loopIdx);
                uint16_T         elDimMapIdx           = rtwCAPI_GetElementDimensionIdx(elemMap, dTypeMapElemMapIdx+loopIdx);
                const char_T*    fieldName             = rtwCAPI_GetElementName(elemMap, dTypeMapElemMapIdx+loopIdx);
 ...
 */
void rt_GetInfoFromOneStructParam(mxArray *valueMat,
                                       PrmStructData *paramStructure,
                                  size_t *paramInfoIndex,
                                       boolean_T modelParam,
                                       int capiIdx,
                                       const char** result)
{
    ...
    if (!mxIsStruct(valueMat)) {
        ...
        *(result) = "non-struct param found where only struct params were expected (rt_GetInfoFromOneStructParam)";
        ...
        {
            size_t numArrayElements = mxGetNumberOfElements(valueMat);
 ...
 *  Create a ParamInfo struct for a non-struct data type.
 */
const char *rt_PrepareToCreateParamInfosForStructParam(mxArray* valueMat, 
                                                  const mxArray* paParamStructs,
                              ...
                              const PrmStructData *paramStructure)
{
    ...
    {
        size_t loopIdx;
        for (loopIdx=0; loopIdx<nStructLeaves+nNonStructDataTypes; loopIdx++) {
            bool structLeaf    = paramInfo[loopIdx].structLeaf;
            bool complex       = (bool)paramInfo[loopIdx].complex;        
            int  dtTransIdx    = paramInfo[loopIdx].dtTransIdx;
            int  dataType      = paramInfo[loopIdx].dataType;
        int  dtSize        = 0;
        int  nEls          = 0;
        int  elSize        = 0;
        int  nParams       = 0;
        ...
        if (!complex) {
                (void)memcpy(address, paramInfo[loopIdx].rVals,nParams*dtSize);
        } else {
            /*
             * Must interleave the real and imaginary parts.  Simulink style.
             */
                size_t elIdx;
                const char *realSrc = (const char *)paramInfo[loopIdx].rVals;
                const char *imagSrc = (const char *)paramInfo[loopIdx].iVals;
            dst = address;

                for (elIdx=0; structLeaf ? elIdx<nEls : elIdx<nParams; elIdx++) {
                /* Copy real part. */

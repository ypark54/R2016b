...
NeParameterData; typedef struct NeuDiagnosticTreeTag NeuDiagnosticTree; typedef
...
NE_OUTPUT_IO_TYPE, NUM_IO_TYPES } NeDsIoType; typedef struct NeDsIoInfoTag {
const char *mName; size_t mM; size_t mN; const char *mUnit; const char
*mIdentifier; } NeDsIoInfo; PMF_DEPLOY_STATIC size_t ne_get_num_io_signals(const
NeDsIoInfo * io_info_ptr, size_t io_info_size) { size_t i=0, num_signals=0; for
(i = 0; i < io_info_size; i++) { num_signals += io_info_ptr[i].mM *
io_info_ptr[i].mN; } return num_signals; } typedef struct NeCgParamsTag
NeCgParams; typedef struct NeCgResultsTag NeCgResults; void pmf_assertion(const
char *message, const char *fileName, int lineNumber); typedef struct
...
SscLoggingSetting mLoggingMode; }; typedef struct NeDaeTag NeDae; typedef struct
...
mIndices; NeParameterVector mReals; } NeParameterInfo; typedef union
...
PmRealVector mZC; PmIntVector mASSERT; PmIntVector mPASSERT; PmRealVector mSO;
...
NE_DAE_METHOD_ZC, NE_DAE_METHOD_ASSERT, NE_DAE_METHOD_PASSERT, NE_DAE_METHOD_SO,
NE_DAE_METHOD_SP, NE_DAE_METHOD_MODE, NE_DAE_METHOD_UDOT_REQ,
NE_DAE_METHOD_DELAYS, NE_DAE_METHOD_DXF_V_X, NE_DAE_METHOD_DUF_V_X,
NE_DAE_METHOD_OBS_EXP, NE_NUM_DAE_METHODS } NeDaeMethodId; typedef enum
...
NeuDiagnosticManager *); typedef PmfMessageId(*NeDaeFunction)(const NeDae *,
const NeSystemInput *); typedef PmfMessageId(*NeDaeLinearizeFcn)(const NeDae *,
const NeSystemInput *, PmRealVector *, NeuDiagnosticManager *); typedef
NeDaeMethodOutput *(*NeDaeMakeOutputFcn)(const NeDae *, PmAllocator *); typedef
void (*NeDaeFreeOutputFcn)(NeDaeMethodOutput *, PmAllocator *); typedef
NeRealFunctor *(*NeRealFunctorFcn)(const NeDae *dae, NeDaeMethodId id,
PmAllocator *allocator); typedef McMatrixFunction
*(*NeDaeMatrixFunctionFcn)(const NeDae *dae, NeDaeMethodId patternId,
NeDaeMethodId prId, PmAllocator *allocator); typedef void
(*NeDaeSetParameters)(const NeDae *, const NeParameterBundle *); typedef struct
NeDaePrivateDataTag NeDaePrivateData; struct NeDaeTag { size_t mNumStates; size_t
...
size_t mRefCount; NeEquationData *mEquationData; NeVariableData *mVariableData;
NeVariableData *mDiscreteData; NeModeData *mMajorModeData; NeZCData *mZCData;
NeAssertData *mAssertData; NeAssertData *mParamAssertData; NeRange *mRanges;
NeRange *mAssertRanges; NeRange *mParamAssertRanges; NeObservableData
*mObservableData; NeParameterInfo mParameterInfo; const PmBoolVector
*mDirectFeedthrough; const PmIntVector *mOutputFunctionMap; size_t
...
*); NeDae* (*mCloneFcn) (const NeDae *); NeDaeMethod
mMethods[NE_NUM_DAE_METHODS]; NeDaeSolve mSolves[NE_DAE_SOLVE_NUM]; NeDaeSolve
mCheck; NeDaeSetParameters mSetParameters; NeDaeFunction mPreTrDiagnoseFcn;
NeDaeFunction mPreDcDiagnoseFcn; NeDaeSolve mAllInputsWorkFcn; NeDaeLinearizeFcn
mResidualsFcn; NeDaeLinearizeFcn mLinearizeFcn; PmfMessageId (*mInitializeFcn)
(NeDae *dae, NeModelParameters mp, NeuDiagnosticManager *mgr); void
(*mSolverFailureNotify) (NeDae *dae, size_t num_consecutive_failures); void
(*mSetupLoggerFcn) (const NeDae *, NeLoggerBuilder *); int32_T (*mLogFcn) (const
NeDae *, const NeSystemInput *, PmRealVector *); void (*mGenerateFcn) (const
NeDae *, const NeCgParams *, NeCgResults *); const SLMatrices **
(*mSwitchedLinearDataFcn) (const NeDae *); NeDaeMakeOutputFcn
mMakeOutput[NE_NUM_DAE_METHODS]; NeDaeFreeOutputFcn
mFreeOutput[NE_NUM_DAE_METHODS]; NeRealFunctorFcn mRealFunctorFcn;
NeDaeMatrixFunctionFcn mMatrixFunctionFcn; void (*mSetOutputFunction) (const
NeDae *, size_t outputFunctionIndex); void (*mStartOutputLoop) (const NeDae *);
NeCustomData* (*mGetCustomDataFcn)(const NeDae *); PmfMessageId
(*mSetCustomDataFcn)(const NeCustomData *, const NeDae *, NeuDiagnosticManager
*mgr); }; PMF_DEPLOY_STATIC PmfMessageId ne_dae_linearize(const NeDae *dae, const
NeSystemInput *input, PmRealVector *LinJacobian, NeuDiagnosticManager *mgr) {
return dae->mLinearizeFcn(dae, input, LinJacobian, mgr); } PMF_DEPLOY_STATIC
PmfMessageId ne_dae_residuals(const NeDae *daePtr, const NeSystemInput *inputPtr,
PmRealVector *residualsRvPtr, NeuDiagnosticManager *mgr) { return
daePtr->mResidualsFcn(daePtr, inputPtr, residualsRvPtr, mgr); } PMF_DEPLOY_STATIC
PmfMessageId ne_dae_call_method(const NeDae *dae, NeDaeMethodId id, const
NeSystemInput *input, NeDaeMethodOutput *output, NeuDiagnosticManager *mgr) {
return dae->mMethods[id](dae, input, output, mgr); } PMF_DEPLOY_STATIC
NeDaeMethodOutput *ne_dae_default_output(const NeDae *dae, NeDaeMethodId id,
PmAllocator *allocator) { return dae->mMakeOutput[id](dae, allocator); }
PMF_DEPLOY_STATIC NeDaeMethodOutput *ne_dae_method_output(const NeDae *dae,
NeDaeMethodId id, const NeSystemInput *input, NeuDiagnosticManager *mgr,
PmAllocator *allocator) { NeDaeMethodOutput *daeOutput =
ne_dae_default_output(dae, id, allocator); ne_dae_call_method(dae, id, input,
daeOutput, mgr); return daeOutput; } PMF_DEPLOY_STATIC PmfMessageId
ne_dae_solve(const NeDae *dae, NeDaeSolveId id, const NeSystemInput *input,
NeuDiagnosticManager *mgr) { return dae->mSolves[id](dae, input, mgr); }
PMF_DEPLOY_STATIC NeSystemInputSizes ne_dae_get_system_input_sizes(const NeDae
*dae) { NeSystemInputSizes sizes; sizes.mSizes[NE_SYSTEM_INPUT_ID_Q] =
dae->mNumMajorModes; sizes.mSizes[NE_SYSTEM_INPUT_ID_M] = dae->mNumModes;
sizes.mSizes[NE_SYSTEM_INPUT_ID_T] = 1; sizes.mSizes[NE_SYSTEM_INPUT_ID_U] =
dae->mNumInputs; sizes.mSizes[NE_SYSTEM_INPUT_ID_V] = dae->mNumInputs;
sizes.mSizes[NE_SYSTEM_INPUT_ID_X] = dae->mNumStates;
sizes.mSizes[NE_SYSTEM_INPUT_ID_D] = dae->mNumDiscreteStates;
sizes.mSizes[NE_SYSTEM_INPUT_ID_E] = dae->mNumSamples;
sizes.mSizes[NE_SYSTEM_INPUT_ID_CR] = dae->mNumRealCache;
sizes.mSizes[NE_SYSTEM_INPUT_ID_CI] = dae->mNumIntCache; return sizes; }
PMF_DEPLOY_STATIC void ne_dae_default_output_free(const NeDae *dae,
NeDaeMethodOutput *output, NeDaeMethodId id, PmAllocator *allocator) {
dae->mFreeOutput[id](output, allocator); } PMF_DEPLOY_STATIC void
ne_dae_get_reference(NeDae *dae) { ++(dae->mRefCount); } PMF_DEPLOY_STATIC void
ne_dae_release_reference(NeDae *dae) { ; if (--(dae->mRefCount) == 0) {
dae->mDestroy(dae); } }
#ifdef  __cplusplus
}
#endif /* __cplusplus */

...
const char _ssBadLevel2InputAccessMsg[] =
  ...
  "input port '%d' in one of its methods. However, the S-function's "
...
const char _ssBadLevel2ReusableInputAccessMsg[] =
  "S-Function '%s' in block '%s' is attempting to access the signal from "
  "input port '%d' in one of its methods. However, this input port is declared "
  "reusable, and its buffer cannot be accessed in the mdlInitializeConditions "
  "and/or mdlUpdate method(s).\n";
...
InputPtrsType _ssGetInputPortSignalPtrsFcn(const SimStruct *S, int ip)
{
    ...
    if ((S)->portInfo.inputs[(ip)].signal.ptrs == NULL) {
        char *dialogMessage = NULL;
        /* In case the user declared SS_REUSABLE_AND_LOCAL/SS_REUSABLE_AND_GLOBAL,
           input buffer may be set to NULL. Call the _ssFatalError to report such error */
        if (ssGetInputPortOptimOpts((S), (ip)) == SS_REUSABLE_AND_GLOBAL ||
            ssGetInputPortOptimOpts((S), (ip)) == SS_REUSABLE_AND_LOCAL) {
            dialogMessage = (char *)malloc(sizeof(_ssBadLevel2ReusableInputAccessMsg)
                                           + strlen(ssGetModelName(S))
                                           + strlen(ssGetPath(S))
                                           + 4*sizeof(char) + 1);
            
            if (dialogMessage != NULL) {
                (void)sprintf(dialogMessage, _ssBadLevel2ReusableInputAccessMsg,
         ...
         */
        dialogMessage = (char *)malloc(sizeof(_ssBadLevel2ZeroDirectFeedMsg)
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_SETUP_RUNTIME_RESOURCES)
          case SS_CALL_MDL_SETUP_RUNTIME_RESOURCES:
            if (ssGetmdlSetupRuntimeResources(S) != NULL) {
                ...
                ssSetErrorStatus(S,"Unexpected call to mdlSetupRuntimeResources");
            }
            break;
#         endif
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_CLEANUP_RUNTIME_RESOURCES)
          case SS_CALL_MDL_CLEANUP_RUNTIME_RESOURCES:
            if (ssGetmdlCleanupRuntimeResources(S) != NULL) {
                ...
                ssSetErrorStatus(S,"Unexpected call to mdlCleanupRuntimeResources");
            }
            break;
#         endif
